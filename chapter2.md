...
(By the way, bit shifting is usually faster than multiplication or division. So if you're timing out by a couple &micro;s, go ahead and replace all instances of `x*2` with `(x<<1)`.)