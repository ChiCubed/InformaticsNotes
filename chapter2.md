...
Ensure that you use the correct format specifier, i.e. to print a long long integer you must use `%lld`. This is one disadvantage of `printf` as opposed to `cout`: with `cout` you don't have to worry about types. I have lost a lot of time in contest because I've accidentally printed my final answer  as a decimal rather than a long long.